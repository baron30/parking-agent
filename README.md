...
flowchart TD
    ...
    subgraph PROGRAM["🐍 程式執行層（5 輪 × ~60s = 實質每分鐘偵測）"]
        ...
        RES -->|"您已完成\n線上預約登記"| VERIFY["🔍 查詢預約記錄\n輸入車牌 → 查詢\n確認日期出現在結果中"]
        RES -->|"車號已於X\n登記預約"| VERIFY2["🔍 查詢預約記錄\n確認舊記錄存在"]
        VERIFY -->|"找到記錄"| OK["✅ 通知 2\n預約成功！"]
        VERIFY -->|"找不到記錄"| NG2["⚠️ 通知 2\n顯示完成\n但記錄未找到"]
        VERIFY2 -->|"找到記錄"| OK2["✅ 通知 2\n預約已確認存在！"]
        VERIFY2 -->|"找不到記錄"| NG3["⚠️ 通知 2\n顯示已登記\n但記錄未找到"]
    end
...
| 未偵測到任何已知完成訊息 | log warning，停止輪詢 | ✅ 通知（請手動確認） |
| 「您已完成線上預約登記」→ 查詢記錄找到 | log info，停止輪詢 | ✅ 通知預約成功 |
| 「您已完成線上預約登記」→ 查詢記錄找不到 | log warning，停止輪詢 | ✅ 通知（請手動確認） |
| 「車號 [車牌] 已於 X 登記預約」→ 查詢記錄找到 | log info，停止輪詢 | ✅ 通知預約已確認存在 |
| 「車號 [車牌] 已於 X 登記預約」→ 查詢記錄找不到 | log warning，停止輪詢 | ✅ 通知（請手動確認） |
